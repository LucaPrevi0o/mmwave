...
void print_help(parser_t* parser) {
  ...
  printf("The following parameters are supported:\n");
  printf("    [PROFILE]\n");
  printf("        id = <profile_id>\n");
  printf("        startFrequency = <start_frequency_in_GHz>\n");
  printf("        frequencySlope = <frequency_slope_in_MHz/us>\n");
  printf("        idleTime = <idle_time_in_us>\n");
  printf("        adcStartTime = <adc_start_time_in_us>\n");
  printf("        rampEndTime = <ramp_end_time_in_us>\n");
  printf("        txStartTime = <tx_start_time_in_us>\n");
  printf("        numAdcSamples = <number_of_adc_samples>\n");
  printf("        adcSamplingFrequency = <adc_sampling_frequency_in_ksps>\n");
  printf("        rxGain = <rx_gain_in_dB>\n\n");
  printf("    [CHIRP]\n");
  printf("        profileId = <profile_id>\n");
  printf("        chirpStartIdx = <chirp_start_index>\n");
  printf("        chirpEndIdx = <chirp_end_index>\n");
  printf("        txEnable = <tx_enable>\n");
  printf("        adcStartTimeVar = <adc_start_time_variable>\n");
  printf("        idleTimeVar = <idle_time_variable>\n");
  printf("        startFreqVar = <start_frequency_variable>\n");
  printf("        freqSlopeVar = <frequency_slope_variable>\n\n");
  printf("    [FRAME]\n");
  printf("        chirpStartIdx = <chirp_start_index>\n");
  printf("        chirpEndIdx = <chirp_end_index>\n");
  printf("        numFrames = <number_of_frames>\n");
  printf("        numLoops = <number_of_loops>\n");
  printf("        framePeriodicity = <frame_periodicity_in_ms>\n\n");
  printf("    [CHANNEL]\n");
  printf("        rxChannelEn = <rx_channel_enable>\n");
  printf("        txChannelEn = <tx_channel_enable>\n");
  printf("        cascading = <cascading>\n\n");
  printf("    [ADC]\n");
  printf("        fmt = <adc_format>\n");
  printf("        b2AdcBits = <adc_bits>\n");
  printf("        b2AdcOutFmt = <adc_output_format>\n");
  printf("        b8FullScaleReducFctr = <full_scale_reduction_factor>\n\n");
  printf("    [DATA]\n");
  printf("        iqSwapSel = <iq_swap_selection>\n");
  printf("        chInterleave = <channel_interleave>\n");
  printf("        rxChannelEn = <rx_channel_enable>\n");
  printf("        adcFmt = <adc_format>\n");
  printf("        adcBits = <adc_bits>\n\n");
  printf("    [LDO]\n");
  printf("        bypass = <ldo_bypass>\n");
  printf("        bypassValue = <bypass_value>\n");
  printf("\n");
}
